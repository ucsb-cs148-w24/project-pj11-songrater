...
export default function FriendsScreen({ navigation }) {
  ...
  const fetchFriend = async () => {
    try {
      ...
      if (!(newname == "")) {
        await fetch(
          `http://127.0.0.1:5000//api/friends_specific?user_id=${UserId}&uname=${newname}`
        )
          .then((data) => {
            setIsLoading(false);
            return data.json();
  ...
  const IfExist = async (Fid) => {
    try {
      await fetch(
        `http://127.0.0.1:5000/api/friend_exists?user_id=${UserId}&fid=${Fid}`
      )
        .then((data) => {
          return data.json();
        })
    ...
    <View style={{ display: "flex", flex: 1, backgroundColor: "#F3F6F7" }}>
      ...
      {searchFriendsState && (
        <View style={{ flex: 8 }}>
          ...
          <View style={{ flex: 6, backgroundColor: "#F3F6F7" }}>
            {!searchFriendsData || searchFriendsData?.length < 1 ? (
              <View style={styles.titleContainer}>
                <Text style={typography.default_bold}>No results</Text>
              </View>
            ) : (
              <FlatList
      ...
      <View style={{ flex: 10, backgroundColor: "#F3F6F7" }}>
        <FlatList
          data={friendsdata}
          keyExtractor={(item) => item.id}
